...
public class JsonUtils {
    ...
    public static JsonNode normalize(
            JsonNode node,
            Function<ValueNode, ValueNode> onValue,
            Function<JsonNode, JsonNode> onContainer) {
        ...
        if (node.isArray()) {
            ...
            objectNode
                    ...
                    .forEachRemaining(
                            key -> {
                                JsonNode normalized =
                                        normalize(objectNode.get(key), onValue, onContainer);
                                if (normalized == null) {
                                    toRemove.add(key);
                                    return;
                                }
    ...
    public static <T extends Comparable<Object>> ArrayNode sortWithType(
            ObjectMapper mapper, ArrayNode target, Class<T> type) {
        return sortWithType(mapper, target, type, Comparable::compareTo);
    }

    public static <T> ArrayNode sortWithType(
            ObjectMapper mapper, ArrayNode target, Class<T> type, Comparator<T> comparator) {
