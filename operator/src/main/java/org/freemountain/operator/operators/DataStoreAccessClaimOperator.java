...
public class DataStoreAccessClaimOperator {
    ...
    @Inject DataStoreAccessClaimCacheEmitter dataStoreAccessClaimCacheEmitter;

    @Inject JobTemplateService jobTemplateService;

    @Inject DataStoreAccessClaimApiClient dataStoreAccessClaimResourceClient;
    ...
    public void init() {
        jobCondition =
                new JobEventConditionUpdater<>(
                        mapper,
                        dataStoreAccessClaimCacheEmitter,
                        dataStoreAccessClaimResourceClient,
                        null);
    }
    ...
    void onEvent(DataStoreAccessClaimLifecycleEvent event) {
        ...
        Optional<DataStoreResource> matchedDataStore =
                dataStoreCache.values().stream()
                        .filter(
                                e ->
                                        e.getMetadata()
                                                .getName()
                                                .equals(spec.getDataStoreSelector().getMatchName()))
                        .findFirst();

        if (matchedSecret.isEmpty() || matchedDataStore.isEmpty()) {
            if (matchedSecret.isEmpty()) {
                LOGGER.errorf(
                        "Could not find secret with name '%s'",
                        spec.getSecretSelector().getMatchName());
            }
            if (matchedDataStore.isEmpty()) {
                LOGGER.errorf(
                        "Could not find dataStore with name '%s'",
                        spec.getDataStoreSelector().getMatchName());
            }
            return;
        }
        ...
        Optional<DataStoreJobTemplate> jobTemplate =
                jobTemplateService.buildCreateUserTemplate(matchedDataStore.get(), resource, user);
